...
import com.andyadc.shopizer.search.utils.IndexConfiguration;
...
public class ObjectIndexerImpl implements IndexWorker {
  ...
  private static final Logger log = LoggerFactory.getLogger(ObjectIndexerImpl.class);
...
/*            if (!StringUtils.isBlank(config.getIndexName())) {
  ...
  public void execute(SearchClient client, String json, String index, String id,
    ...
    try {
      ...
      com.andyadc.shopizer.search.services.GetResponse r = searchDelegate.getObject(index, id);
      if (r != null) {
